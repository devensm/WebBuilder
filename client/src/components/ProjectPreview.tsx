...
const injectPreview = (html:string)=>{
  ...
  return (
    <div className='relative h-full bg-gray-900 flex-1 rounded-x1 overflow-hidden max-sm:ml-2'>
        {project.current_code ? (
            <>
            <iframe 
            ref={iframeRef}
            srcDoc = {injectPreview(project.current_code)}
            className={`h-full max-sm:w-full ${resolutions[device]} mx-auto transitions-all`}
            />
