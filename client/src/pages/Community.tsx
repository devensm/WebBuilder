          ...
          <div className='py-10 min-h-[80vh]'>
            ...
            <div className='flex flex-wrap gap-3.5'>
              {projects.map((project)=>(
                <Link key={project.id} to={`/view/${project.id}`} target = '_blank' className='w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border borfer-gray-700 roundedlg overflow-hidden group hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duration-300'>
                  ...
                  <div className='p-4 text-white bg-linear-180 from-transparent group-hover:from-indigo-950 to-transparent transition-colors'>
                    ...
                    <div className='flex justify-between items-center mt-6'>
                      ...
                      <div className='flex gap-3 text-white text=sm'>
                        <button className='px-3 py-1.5 bg-white/10 hover:bg-while/15 rounded-nd transition-colors flex items-center gap-2'>
                          <span className='bg-gray-200 size-4.5 rounded-full text-vlack font-semibold flex items-center justify-center'>{project.user?.name?.slice(0,1)}</span>
                          {project.user?.name}
                        </button>
                      </div>
                    </div>
